...
def normalize_string(input_str):
    # Para a função find_responsible
    if pd.isnull(input_str):  # Verifica se o valor é nulo
        return "empty"
...
def chat_bot(HUGGING_FACE_KEY, txt_file_path):
    ...
    while True:
        ...
        # Verificar se a pergunta é sobre o responsável
        if "responsible" in query.lower():
            name_to_find = normalize_string(query.split("for")[-1].strip())
            responsible, phone, email = find_responsible(name_to_find, df)

            if responsible != "empty":
                print(f"Chatbot: The responsible for {name_to_find} is {responsible}. Contact: {phone}, Email: {email}")
            else:
                print(f"Chatbot: Sorry, I couldn't find a match for {name_to_find}.")
        else:
            print(f"Chatbot: {bot_response}")
